...
class FriendlyErrorHandler(IErrorHandler):
    ...
    def add_custom_error_handler(
        self, error_type: str, message: str, suggestions: List[str], severity: str = "medium"
    ) -> None:
        """添加自定义错误处理器"""
        self.error_suggestions[error_type] = {"message": message, "suggestions": suggestions, "severity": severity}
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_categories": {
                ...
                "network_errors": len(
                    [k for k in self.error_suggestions.keys() if "connection" in k.lower() or "timeout" in k.lower()]
                ),
            },
        }
