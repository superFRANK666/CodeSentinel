...
class HybridCodeAnalyzer(BaseCodeAnalyzer, ICodeAnalyzer):
    ...
    def _is_sensitive_file(self, file_path: str) -> bool:
        """判断是否为敏感文件"""
        sensitive_patterns = ["config", "secret", "key", "password", "credential", "auth", "token", "private", "admin"]
    ...
    def _merge_results(self, local_result: AnalysisResult, ai_result: AnalysisResult) -> AnalysisResult:
        ...
        # 合并推荐建议
        all_recommendations = list(set(local_result.recommendations + ai_result.recommendations))
    ...
    def get_analyzer_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components": {"local": local_info, "ai": ai_info},
            "features": ["智能分析策略", "双重验证机制", "动态置信度评估", "最佳性能和准确性平衡", "自动降级处理"],
...
class AnalysisStrategy:
    ...
    @staticmethod
    def should_use_ai_analysis(
        file_info: Dict[str, Any], local_result: AnalysisResult, confidence_threshold: float
    ) -> bool:
        """统一判断是否需要AI深度分析"""
        ...
        high_risk_vulnerabilities = [
            vuln
            for vuln in local_result.vulnerabilities
        ...
        low_confidence_vulnerabilities = [
            vuln for vuln in local_result.vulnerabilities if vuln.confidence < confidence_threshold
        ]
    ...
    @staticmethod
    def should_use_ai_heavy_analysis(file_info: Dict[str, Any], local_result: AnalysisResult) -> bool:
    ...
    def estimate_analysis_complexity(file_path: Path, content: str) -> Dict[str, Any]:
        ...
        complexity_factors["analysis_difficulty"] = (
            "high" if complexity_score > 100 else "medium" if complexity_score > 50 else "low"
        )

        return complexity_factors

    @staticmethod
    def select_optimal_analyzer(file_info: Dict[str, Any], user_preferences: Dict[str, Any]) -> str:
        """选择最优的分析器"""
        # 用户偏好
