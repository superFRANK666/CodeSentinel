...
class DependencyContainer:
    ...
    def _dict_to_config(self, config_dict: Dict[str, Any]) -> AppConfig:
        ...
        try:
            ...
            # 确保analyzer字典有所有必需的字段
            analyzer_config = AnalyzerConfig(
                severity_threshold=analyzer_dict.get("severity_threshold", "low"),
                max_file_size=analyzer_dict.get("max_file_size", 1024),
                concurrent_limit=analyzer_dict.get("concurrent_limit", 5),
                cache_enabled=analyzer_dict.get("cache_enabled", True),
                cache_ttl=analyzer_dict.get("cache_ttl", 3600),
                ai_model=analyzer_dict.get("ai_model", "gpt-4o-mini"),
                api_timeout=analyzer_dict.get("api_timeout", 60),
                max_retries=analyzer_dict.get("max_retries", 3),
            )
    ...
    async def _initialize_components(self) -> None:
        ...
        # 缓存管理器
        if not self.is_registered(ICacheManager) and self._config is not None:
            # 检查配置类型并安全访问cache_enabled
            cache_enabled = False
            cache_ttl = 3600

            try:
                if hasattr(self._config, 'analyzer') and hasattr(self._config.analyzer, 'cache_enabled'):
                    cache_enabled = self._config.analyzer.cache_enabled
                    cache_ttl = getattr(self._config.analyzer, 'cache_ttl', 3600)
                elif isinstance(self._config, dict):
                    analyzer_config = self._config.get('analyzer', {})
                    cache_enabled = analyzer_config.get('cache_enabled', True)
                    cache_ttl = analyzer_config.get('cache_ttl', 3600)
                else:
                    # 使用默认值
                    cache_enabled = True
                    cache_ttl = 3600
            ...
            if cache_enabled:
                ...
                cache_manager = FileCacheManager(ttl=cache_ttl)
        ...
        # 代码隐私管理器
        if not self.is_registered(ICodePrivacyManager) and self._config is not None:
            # 安全访问隐私检查配置
            privacy_check_enabled = False
            try:
                if hasattr(self._config, 'security') and hasattr(self._config.security, 'enable_privacy_check'):
                    privacy_check_enabled = self._config.security.enable_privacy_check
                elif isinstance(self._config, dict):
                    security_config = self._config.get('security', {})
                    privacy_check_enabled = security_config.get('enable_privacy_check', True)
                else:
                    privacy_check_enabled = True
            except Exception:
                privacy_check_enabled = True

            if privacy_check_enabled:
    ...
    async def _initialize_analyzers(self) -> None:
        ...
        # 安全获取并发限制配置
        concurrent_limit = 5  # 默认值
        try:
            if self._config is not None:
                if hasattr(self._config, 'analyzer') and hasattr(self._config.analyzer, 'concurrent_limit'):
                    concurrent_limit = self._config.analyzer.concurrent_limit
                elif isinstance(self._config, dict):
                    analyzer_config = self._config.get('analyzer', {})
                    concurrent_limit = analyzer_config.get('concurrent_limit', 5)
        except Exception:
            concurrent_limit = 5
        ...
        if ai_enabled:
            ...
            try:
                if self._config is not None:
                    if hasattr(self._config, 'analyzer'):
                        if hasattr(self._config.analyzer, 'ai_model'):
                            ai_model = self._config.analyzer.ai_model
                        if hasattr(self._config.analyzer, 'api_timeout'):
                            api_timeout = self._config.analyzer.api_timeout
                        if hasattr(self._config.analyzer, 'max_retries'):
                            max_retries = self._config.analyzer.max_retries
                        if hasattr(self._config.analyzer, 'base_url'):
                            base_url = self._config.analyzer.base_url
                    elif isinstance(self._config, dict):
                        analyzer_config = self._config.get('analyzer', {})
                        ai_model = analyzer_config.get('ai_model', 'gpt-4o-mini')
                        api_timeout = analyzer_config.get('api_timeout', 60)
                        max_retries = analyzer_config.get('max_retries', 3)
                        base_url = analyzer_config.get('base_url')
            except Exception:
                # 使用默认值
                pass
