...
from ..core.analyzers.taint_analyzer import TaintAnalyzer, TaintType, TaintFlow
...
class LocalCodeAnalyzer(BaseCodeAnalyzer, ICodeAnalyzer):
                ...
                for target in node.targets:
                    ...
                    if target_name:
                        self.assignments.append({
                            'target': target_name,
                            'line': node.lineno,
                            'value_type': self._get_node_type(node.value)
                        })
                self.generic_visit(node)
    ...
    async def _detect_vulnerabilities(self, content: str, file_path: Path,
        ...
        for detector in self.detectors:
            task = self._run_detector(
                detector, content, file_path, pre_analysis, ast_analysis
            )
        ...
        filtered_vulnerabilities = [
            ...
            if self._should_include_by_severity(vuln.severity, severity_filter)
        ]
    ...
    def _should_include_by_severity(self, vulnerability_severity: SeverityLevel, filter_severity: SeverityLevel) -> bool:
        """根据严重度判断是否应该包含该漏洞"""
        severity_order = {
            SeverityLevel.LOW: 1,
    ...
    async def _run_detector(self, detector, content: str, file_path: Path,
                            pre_analysis: Dict[str, Any], ast_analysis: Dict[str, Any]) -> List[Vulnerability]:
        """运行单个检测器"""
        try:
            ...
            vulnerabilities = await loop.run_in_executor(
                self.executor,
                detector.detect_vulnerabilities,
                content, file_path, pre_analysis, ast_analysis
            )
...
class SQLInjectionDetector:
    ...
    def detect_vulnerabilities(self, content: str, file_path: Path,
                               pre_analysis: Dict[str, Any], ast_analysis: Dict[str, Any]) -> List[Vulnerability]:
        """检测SQL注入漏洞"""
        vulnerabilities = []
        ...
        if self.taint_analyzer and 'taint_analysis' in ast_analysis:
            ...
            sql_injection_flows = [
                ...
                if self._is_sql_related_sink(flow.sink)
            ]
...
class CommandInjectionDetector:
    ...
    def detect_vulnerabilities(self, content: str, file_path: Path,
        ...
        if self.taint_analyzer and 'taint_analysis' in ast_analysis:
            taint_flows = ast_analysis['taint_analysis']['taint_flows']
            command_injection_flows = [
                flow for flow in taint_flows
                if self._is_command_related_sink(flow.sink)
            ]
    ...
    def _is_command_related_sink(self, sink_function: str) -> bool:
        ...
        command_related_functions = [
            ...
            'popen', 'spawn', 'fork', 'exec', 'eval', 'exec', 'shell_exec'
        ]
        return any(cmd_func in sink_function for cmd_func in command_related_functions)

    def _is_command_injection_risk(self, func_name: str, call_info: Dict[str, Any], ast_analysis: Dict[str, Any]) -> bool:
        """检查是否存在命令注入风险"""
        dangerous_functions = [
            'os.system', 'subprocess.call', 'subprocess.run', 'subprocess.Popen',
            'popen', 'spawn', 'fork', 'exec', 'eval', 'exec'
        ]
...
class XSSVulnerabilityDetector:
    ...
    def detect_vulnerabilities(self, content: str, file_path: Path,
        ...
        # 检查危险函数调用
        dangerous_funcs = [
            'render_template_string', 'mark_safe', 'innerHTML',
