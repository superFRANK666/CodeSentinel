...
class RegexPrivacyManager(ICodePrivacyManager):
    ...
    def __init__(self):
        ...
        self.sensitive_patterns = {
            ...
            # 令牌
            'token': re.compile(
                r'(?i)(?:token|auth[_-]?token|bearer)\s*[:=]\s*["\']?([a-zA-Z0-9_\-\.]{20,})["\']?',
                re.IGNORECASE
            ),
            # 私钥
        ...
        for pattern_name in patterns_to_use:
            if pattern_name in self.sensitive_patterns:
                ...
                sanitized_content = pattern.sub(
                    lambda m: self._create_replacement(m, pattern_name, replacement),
                    sanitized_content
                )
    ...
    def get_patterns_info(self) -> Dict[str, Any]:
        ...
        for name, pattern in self.sensitive_patterns.items():
            patterns_info[name] = {
                ...
                'privacy_levels': [
                    level for level, info in self.privacy_levels.items()
                    if name in info['patterns']
                ]
            }
    ...
    def create_privacy_report(self, content: str) -> Dict[str, Any]:
        ...
        for level in ['none', 'basic', 'full']:
            ...
            sanitized_results[level] = {
                ...
                'sanitized_content_preview': sanitized_content[:500] + "..." if len(sanitized_content) > 500 else sanitized_content
            }
