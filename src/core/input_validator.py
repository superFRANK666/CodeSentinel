...
class FileValidator:
    ...
    def __init__(self):
        ...
        self.allowed_extensions = {
            '.py', '.pyw', '.pyx', '.pxd',  # Python文件
            '.js', '.ts', '.jsx', '.tsx',  # JavaScript/TypeScript
            '.java', '.kt', '.scala',      # JVM语言
            '.go', '.rs', '.c', '.cpp', '.h', '.hpp',  # 系统编程语言
            '.rb', '.php', '.swift', '.m', '.mm',  # 其他语言
            '.html', '.css', '.scss', '.sass', '.less',  # Web前端
            '.sql', '.xml', '.json', '.yaml', '.yml', '.toml',  # 数据文件
            '.md', '.txt', '.rst'  # 文档文件
        }
    ...
    def validate_file_path(self, file_path: Path) -> ValidationResult:
        ...
        try:
            ...
            if ".." in str(file_path) or not str(resolved_path).startswith(str(Path.cwd().resolve())):
                return ValidationResult(
                    is_valid=False,
                    message=f"检测到潜在的路径遍历攻击: {file_path}",
                    risk_level="critical"
                )
    ...
    def validate_file_size(self, file_path: Path) -> ValidationResult:
        ...
        try:
            ...
            if file_size < self.min_file_size_bytes:
                return ValidationResult(
                    is_valid=False,
                    message=f"文件太小({file_size}字节),可能不是有效的代码文件",
                    risk_level="medium"
                )
            ...
            return ValidationResult(
                is_valid=True,
                message=f"文件大小验证通过（{file_size_mb:.1f} MB）",
                risk_level="low"
            )
        ...
        try:
            ...
            try:
                ...
                if current_uid and file_stat.st_uid != current_uid:
                    return ValidationResult(
                        is_valid=True,
                        message=f"文件所有者与当前用户不同（UID: {file_stat.st_uid}）",
                        risk_level="low"
                    )
